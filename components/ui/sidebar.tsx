...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:inset-s-1/2 after:w-0.5 hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2",
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium text-sidebar-foreground tabular-nums select-none peer-hover/menu-button:text-sidebar-accent-foreground peer-data-[size=default]/menu-button:top-1.5",
        className,
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  // Deterministic placeholder width between 50% and 90%, derived from the
  // element's own id. Random widths differed between the server and client
  // render, which produced a hydration mismatch.
